...
class ArgumentsParser {
 ...
 protected:
  ...
  class ArgumentT : public IArgument {
   public:
    ArgumentT(DataType& p_target, const std::string& p_representStringShort,
              const std::string& p_representString,
              const std::string& p_description, bool p_followedValue,
              const DataType& p_switchAsValue, bool p_isRequired)
        : m_value(p_target),
  ...
  void AddRequiredOption(DataType& p_target,
    ...
    m_arguments.emplace_back(std::shared_ptr<IArgument>(new ArgumentT<DataType>(
        p_target, p_representStringShort, p_representString, p_description,
        true, DataType(), true)));
  }
