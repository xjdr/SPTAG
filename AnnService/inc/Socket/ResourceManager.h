...
class ResourceManager
    : public std::enable_shared_from_this<ResourceManager<ResourceType>> {
 public:
  typedef std::function<void(std::shared_ptr<ResourceType>)> TimeoutCallback;

  ResourceManager()
      : m_nextResourceID(1), m_isStopped(false), m_timeoutItemCount(0) {
  ...
  ResourceID Add(const std::shared_ptr<ResourceType>& p_resource,
    ...
    if (p_timeoutMilliseconds > 0) {
      ...
      item->m_expireTime =
          m_clock.now() + std::chrono::milliseconds(p_timeoutMilliseconds);
 ...
 private:
  struct ResourceItem {
    ResourceItem() : m_resourceID(c_invalidResourceID) {}
