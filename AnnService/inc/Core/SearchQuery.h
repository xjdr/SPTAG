...
class QueryResult {
 public:
  ...
  QueryResult() : m_target(nullptr), m_resultNum(0), m_withMeta(false) {}

  QueryResult(const void* p_target, int p_resultNum, bool p_withMeta)
      : m_target(nullptr), m_resultNum(0), m_withMeta(false) {
    Init(p_target, p_resultNum, p_withMeta);
  }

  QueryResult(const void* p_target, int p_resultNum,
  ...
  QueryResult(const QueryResult& p_other)
    ...
    if (m_resultNum > 0) {
      ...
      std::memcpy(m_results.get(), p_other.m_results.get(),
                  sizeof(BasicResult) * m_resultNum);

      if (m_withMeta) {
        m_metadatas.reset(new ByteArray[m_resultNum]);
        std::copy(p_other.m_metadatas.get(),
                  p_other.m_metadatas.get() + m_resultNum, m_metadatas.get());
      }
    }
  }
  ...
  inline int GetResultNum() const { return m_resultNum; }

  inline const void* GetTarget() { return m_target; }

  inline void SetTarget(const void* p_target) { m_target = p_target; }
  ...
  inline BasicResult* GetResults() const { return m_results.get(); }

  inline bool WithMeta() const { return m_withMeta; }
  ...
  iterator begin() { return m_results.get(); }

  iterator end() { return m_results.get() + m_resultNum; }

  const_iterator begin() const { return m_results.get(); }

  const_iterator end() const { return m_results.get() + m_resultNum; }
