...
inline bool operator<(const BasicResult& lhs, const BasicResult& rhs) {
  return ((lhs.Dist < rhs.Dist) ||
          ((lhs.Dist == rhs.Dist) && (lhs.VID < rhs.VID)));
}
...
class QueryResultSet : public QueryResult {
 public:
  ...
  inline void SetTarget(const T* p_target) { m_target = p_target; }

  inline const T* GetTarget() const {
    return reinterpret_cast<const T*>(m_target);
  }

  inline float worstDist() const { return m_results[0].Dist; }

  bool AddPoint(const int index, float dist) {
    if (dist < m_results[0].Dist ||
        (dist == m_results[0].Dist && index < m_results[0].VID)) {
