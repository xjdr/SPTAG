...
#define REPEAT(type, ctype, delta, load, exec, acc, result) \
  ...
  static float ComputeL2Distance(const std::int8_t* pX, const std::int8_t* pY,
                                 int length) {
  ...
  static float ComputeL2Distance(const std::uint8_t* pX, const std::uint8_t* pY,
                                 int length) {
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd32) {
      REPEAT(__m256i, __m256i, 32, _mm256_loadu_si256, _mm256_sqdf_epu8,
             _mm256_add_ps, diff256)
    }
    __m128 diff128 = _mm_add_ps(_mm256_castps256_ps128(diff256),
                                _mm256_extractf128_ps(diff256, 1));
  ...
  static float ComputeL2Distance(const std::int16_t* pX, const std::int16_t* pY,
                                 int length) {
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd16) {
      REPEAT(__m256i, __m256i, 16, _mm256_loadu_si256, _mm256_sqdf_epi16,
             _mm256_add_ps, diff256)
    }
    __m128 diff128 = _mm_add_ps(_mm256_castps256_ps128(diff256),
                                _mm256_extractf128_ps(diff256, 1));
  ...
  static float ComputeL2Distance(const float* pX, const float* pY, int length) {
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd16) {
      REPEAT(__m256, const float, 8, _mm256_loadu_ps, _mm256_sqdf_ps,
             _mm256_add_ps, diff256)
      REPEAT(__m256, const float, 8, _mm256_loadu_ps, _mm256_sqdf_ps,
             _mm256_add_ps, diff256)
    }
  ...
  /*
              template<typename T>
              static float ComputeCosineDistance(const T *pX, const T *pY, int
     length) {
                  float diff = 0;
                  const T* pEnd1 = pX + length;
  ...
  */
  static float ComputeCosineDistance(const std::int8_t* pX,
                                     const std::int8_t* pY, int length) {
    const std::int8_t* pEnd32 = pX + ((length >> 5) << 5);
    const std::int8_t* pEnd16 = pX + ((length >> 4) << 4);
    const std::int8_t* pEnd4 = pX + ((length >> 2) << 2);
    const std::int8_t* pEnd1 = pX + length;
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd32) {
      REPEAT(__m256i, __m256i, 32, _mm256_loadu_si256, _mm256_mul_epi8,
             _mm256_add_ps, diff256)
    }
    __m128 diff128 = _mm_add_ps(_mm256_castps256_ps128(diff256),
                                _mm256_extractf128_ps(diff256, 1));
  ...
  static float ComputeCosineDistance(const std::uint8_t* pX,
                                     const std::uint8_t* pY, int length) {
    const std::uint8_t* pEnd32 = pX + ((length >> 5) << 5);
    const std::uint8_t* pEnd16 = pX + ((length >> 4) << 4);
    const std::uint8_t* pEnd4 = pX + ((length >> 2) << 2);
    const std::uint8_t* pEnd1 = pX + length;
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd32) {
      REPEAT(__m256i, __m256i, 32, _mm256_loadu_si256, _mm256_mul_epu8,
             _mm256_add_ps, diff256)
    }
    __m128 diff128 = _mm_add_ps(_mm256_castps256_ps128(diff256),
                                _mm256_extractf128_ps(diff256, 1));
  ...
  static float ComputeCosineDistance(const std::int16_t* pX,
                                     const std::int16_t* pY, int length) {
    const std::int16_t* pEnd16 = pX + ((length >> 4) << 4);
    const std::int16_t* pEnd8 = pX + ((length >> 3) << 3);
    const std::int16_t* pEnd4 = pX + ((length >> 2) << 2);
    const std::int16_t* pEnd1 = pX + length;
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd16) {
      REPEAT(__m256i, __m256i, 16, _mm256_loadu_si256, _mm256_mul_epi16,
             _mm256_add_ps, diff256)
    }
    __m128 diff128 = _mm_add_ps(_mm256_castps256_ps128(diff256),
                                _mm256_extractf128_ps(diff256, 1));
  ...
  static float ComputeCosineDistance(const float* pX, const float* pY,
                                     int length) {
    const float* pEnd16 = pX + ((length >> 4) << 4);
    const float* pEnd4 = pX + ((length >> 2) << 2);
    const float* pEnd1 = pX + length;
    ...
    __m256 diff256 = _mm256_setzero_ps();
    while (pX < pEnd16) {
      REPEAT(__m256, const float, 8, _mm256_loadu_ps, _mm256_mul_ps,
             _mm256_add_ps, diff256)
      REPEAT(__m256, const float, 8, _mm256_loadu_ps, _mm256_mul_ps,
             _mm256_add_ps, diff256)
    }
  ...
  template <typename T>
  static inline float ComputeDistance(const T* p1, const T* p2, int length,
  ...
  static inline float ConvertCosineSimilarityToDistance(float cs) {
    // Cosine similarity is in [-1, 1], the higher the value, the closer are the
    // two vectors.
    // However, the tree is built and searched based on "distance" between two
    // vectors, that's >=0. The smaller the value, the closer are the two
    // vectors.
    // So we do a linear conversion from a cosine similarity to a distance
    // value.
    return 1 - cs;  //[1, 3]
  }
...
template <typename T>
float (*DistanceCalcSelector(SPTAG::DistCalcMethod p_method))(const T*,
                                                              const T*, int) {
  switch (p_method) {
