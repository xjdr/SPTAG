...
class DataUtils {
 public:
  ...
  static void ProcessTSVData(int id, int threadbase, std::uint64_t blocksize,
    ...
    outputStream.open(outfile + std::to_string(id + threadbase),
                      std::ofstream::binary);
    metaStream_out.open(outmetafile + std::to_string(id + threadbase),
                        std::ofstream::binary);
    metaStream_index.open(outmetaindexfile + std::to_string(id + threadbase),
                          std::ofstream::binary);
    if (!outputStream.is_open() || !metaStream_out.is_open() ||
        !metaStream_index.is_open()) {
      std::cerr << "unable to open output file " << outfile << " "
                << outmetafile << " " << outmetaindexfile << std::endl;
    ...
    std::cout << "Begin thread " << id << " begin at:" << (startpos + totalread)
              << std::endl;
    while (!inputStream.eof() && totalread <= blocksize) {
      std::getline(inputStream, currentLine);
      if (currentLine.length() <= enter_size ||
          (index = Utils::ProcessLine(currentLine, arr, D, base,
                                      distCalcMethod)) < 0) {
        totalread += currentLine.length() + enter_size;
        continue;
      }
    ...
    std::cout << "Finish Thread[" << id << ", " << sampleSize
              << "] at:" << (startpos + totalread) << std::endl;
  }

  static void MergeData(int threadbase, std::string outfile,
                        std::string outmetafile, std::string outmetaindexfile,
                        std::atomic_int &numSamples, int D) {
    ...
    for (int i = 0; i < threadbase; i++) {
      ...
      inputStream.read((char *)offsets,
                       sizeof(std::uint64_t) * (partSamples + 1));
      inputStream.close();
      remove(file.c_str());
    ...
    std::cout << "numSamples:" << numSamples << " metaSamples:" << metaSamples
              << " D:" << D << std::endl;
  }

  static bool MergeIndex(const std::string &p_vectorfile1,
                         const std::string &p_metafile1,
                         const std::string &p_metaindexfile1,
                         const std::string &p_vectorfile2,
                         const std::string &p_metafile2,
                         const std::string &p_metaindexfile2) {
...
#define MergeVector(inputStream, vectorFile, R, C)                \
  ...
  if (!inputStream.is_open()) {                                   \
    std::cout << "Cannot open vector file: " << vectorFile << "!" \
              << std::endl;                                       \
    return false;                                                 \
  }                                                               \
...
#define MergeMeta(inputStream, metaFile)                                    \
  ...
  inputStream.close();
      ...
      MergeMeta(inputStream1, p_metafile1) MergeMeta(inputStream2, p_metafile2)
#undef MergeMeta
          outputStream.close();
      delete[] buf;
...
#define MergeMetaIndex(inputStream, metaIndexFile)                          \
  ...
  if (!inputStream.is_open()) {                                             \
    std::cout << "Cannot open meta index file: " << metaIndexFile << "!"    \
              << std::endl;                                                 \
    return false;                                                           \
  }                                                                         \
  inputStream.read((char *)&partSamples, sizeof(int));                      \
...
#undef MergeMetaIndex
  ...
  template <typename T>
  static void ParseData(std::string filenames, std::string outfile,
                        std::string outmetafile, std::string outmetaindexfile,
    ...
    std::vector<std::string> inputFileNames =
        Helper::StrUtils::SplitString(filenames, ",");
...
#pragma omp parallel for
      for (int i = 0; i < threadnum; i++) {
        ProcessTSVData<T>(i, threadbase, blocksize, inputFileName, outfile,
                          outmetafile, outmetaindexfile, numSamples, D,
                          distCalcMethod);
      }
      threadbase += threadnum;
    }
    MergeData(threadbase, outfile, outmetafile, outmetaindexfile, numSamples,
              D);
  }
};
}
