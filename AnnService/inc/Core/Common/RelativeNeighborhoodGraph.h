...
class RelativeNeighborhoodGraph : public NeighborhoodGraph {
 public:
  void RebuildNeighbors(VectorIndex* index, const int node, int* nodes,
                        const BasicResult* queryResults, const int numResults) {
    int count = 0;
    for (int j = 0; j < numResults && count < m_iNeighborhoodSize; j++) {
      ...
      for (int k = 0; k < count; k++) {
        if (index->ComputeDistance(index->GetSample(nodes[k]),
  ...
  void InsertNeighbors(VectorIndex* index, const int node, int insertNode,
                       float insertDist) {
    ...
    for (int k = 0; k < m_iNeighborhoodSize; k++) {
      ...
      if (tmpNode < 0) {
        bool good = true;
        for (int t = 0; t < k; t++) {
          if (index->ComputeDistance(index->GetSample(insertNode),
      ...
      float tmpDist = index->ComputeDistance(index->GetSample(node),
                                             index->GetSample(tmpNode));
      if (insertDist < tmpDist ||
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < samples; i++) {
      ...
      for (int y = 0; y < m_iGraphSize; y++) {
        ...
        float dist =
            index->ComputeDistance(index->GetSample(x), index->GetSample(y));
