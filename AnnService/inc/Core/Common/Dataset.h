 ...
 public:
  ...
  void Initialize(int rows_, int cols_, T* data_ = nullptr,
    ...
    if (data_ == nullptr || !transferOnwership_) {
      ...
      if (data_ != nullptr)
        memcpy(data, data_, ((size_t)rows) * cols * sizeof(T));
      else
        std::memset(data, -1, ((size_t)rows) * cols * sizeof(T));
    }
  }
  void SetR(int R_) {
    if (R_ >= rows)
  ...
  void AddBatch(const T* pData, int num) {
    dataIncremental.insert(dataIncremental.end(), pData,
                           pData + ((size_t)num) * cols);
  }
  ...
  bool Save(std::string sDataPointsFileName) {
    ...
    std::cout << "Save Data (" << CR << ", " << cols << ") Finish!"
              << std::endl;
    return true;
  }
  ...
  bool Load(std::string sDataPointsFileName) {
    ...
    fclose(fp);
    std::cout << "Load Data (" << rows << ", " << cols << ") Finish!"
              << std::endl;
    return true;
  }
  ...
  bool Refine(const std::vector<int>& indices,
              std::string sDataPointsFileName) {
    ...
    for (int i = 0; i < R; i++) {
      if (indices[i] < rows)
        ...
        fwrite(dataIncremental.data() + ((size_t)(indices[i] - rows)) * cols,
               sizeof(T) * cols, 1, fp);
    }
    fclose(fp);

    std::cout << "Save Refine Data (" << R << ", " << cols << ") Finish!"
              << std::endl;
    return true;
  }
};
