...
class NeighborhoodGraph {
 public:
  ...
  virtual void InsertNeighbors(VectorIndex* index, const int node,
                               int insertNode, float insertDist) = 0;

  virtual void RebuildNeighbors(VectorIndex* index, const int node, int* nodes,
                                const BasicResult* queryResults,
                                const int numResults) = 0;

  virtual float GraphAccuracyEstimation(
      VectorIndex* index, const int samples,
      const std::unordered_map<int, int>* idmap = nullptr) = 0;

  template <typename T>
  void BuildGraph(VectorIndex* index,
                  const std::unordered_map<int, int>* idmap = nullptr) {
    ...
    {
      COMMON::Dataset<float> NeighborhoodDists(m_iGraphSize,
                                               m_iNeighborhoodSize);
      std::vector<std::vector<int>> TptreeDataIndices(
          m_iTPTNumber, std::vector<int>(m_iGraphSize));
      std::vector<std::vector<std::pair<int, int>>> TptreeLeafNodes(
          m_iTPTNumber, std::vector<std::pair<int, int>>());
...
#pragma omp parallel for schedule(dynamic)
      for (int i = 0; i < m_iTPTNumber; i++) {
        ...
        std::random_shuffle(TptreeDataIndices[i].begin(),
                            TptreeDataIndices[i].end());
...
#pragma omp parallel for schedule(dynamic)
        for (int j = 0; j < TptreeLeafNodes[i].size(); j++) {
          ...
          if (omp_get_thread_num() == 0)
            std::cout << "\rProcessing Tree " << i << ' '
                      << j * 100 / TptreeLeafNodes[i].size() << '%';
          for (int x = start_index; x < end_index; x++) {
            for (int y = x + 1; y <= end_index; y++) {
              int p1 = TptreeDataIndices[i][x];
              int p2 = TptreeDataIndices[i][y];
              float dist = index->ComputeDistance(index->GetSample(p1),
                                                  index->GetSample(p2));
              if (idmap != nullptr) {
  ...
  template <typename T>
  void RefineGraph(VectorIndex* index,
                   const std::unordered_map<int, int>* idmap = nullptr) {
    m_iCEF *= m_iCEFScale;
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < m_iGraphSize; i++) {
      ...
      if (i % 1000 == 0)
        std::cout << "\rRefine 1 " << (i * 100 / m_iGraphSize) << "%";
    }
    std::cout << "Refine RNG, graph acc:"
              << GraphAccuracyEstimation(index, 100, idmap) << std::endl;
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < m_iGraphSize; i++) {
      RefineNode<T>(index, i, false);
      if (i % 1000 == 0)
        std::cout << "\rRefine 2 " << (i * 100 / m_iGraphSize) << "%";
    }
    std::cout << "Refine RNG, graph acc:"
              << GraphAccuracyEstimation(index, 100, idmap) << std::endl;
    ...
    if (idmap != nullptr) {
      for (auto iter = idmap->begin(); iter != idmap->end(); iter++)
        if (iter->first < 0) {
          m_pNeighborhoodGraph[-1 - iter->first][m_iNeighborhoodSize - 1] =
              -2 - iter->second;
        }
    }
  }
  ...
  template <typename T>
  ErrorCode RefineGraph(VectorIndex* index, std::vector<int>& indices,
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < R; i++) {
      ...
      for (int j = 0; j < m_iNeighborhoodSize; j++) {
        if (nodes[j] < 0)
          ...
          nodes[j] = reverseIndices[nodes[j]];
      }
      if (idmap == nullptr || idmap->find(-1 - indices[i]) == idmap->end())
        continue;
    ...
    for (int i = 0; i < R; i++) {
      graphOut.write((char*)m_pNeighborhoodGraph[indices[i]],
                     sizeof(int) * m_iNeighborhoodSize);
    }
    graphOut.close();
  ...
  void RefineNode(VectorIndex* index, const int node, bool updateNeighbors) {
    COMMON::QueryResultSet<T> query((const T*)index->GetSample(node),
                                    m_iCEF + 1);
    index->SearchIndex(query);
    RebuildNeighbors(index, node, m_pNeighborhoodGraph[node],
                     query.GetResults(), m_iCEF + 1);
  ...
  template <typename T>
  void PartitionByTptree(VectorIndex* index, std::vector<int>& indices,
    ...
    if (last - first <= m_iTPTLeafSize) {
      ...
      std::vector<float> weight(m_numTopDimensionTPTSplit),
          bestweight(m_numTopDimensionTPTSplit);
  ...
  bool LoadGraph(std::string sGraphFilename) {
    ...
    fclose(fp);
    std::cout << "Load Graph (" << m_iGraphSize << "," << m_iNeighborhoodSize
              << ") Finish!" << std::endl;
    return true;
  }
  ...
  bool SaveGraph(std::string sGraphFilename) const {
    ...
    fclose(fp);
    std::cout << "Save Graph (" << m_iGraphSize << "," << m_iNeighborhoodSize
              << ") Finish!" << std::endl;
    return true;
  }

  inline void AddBatch(int num) {
    m_pNeighborhoodGraph.AddBatch(num);
    m_iGraphSize += num;
    m_dataUpdateLock.resize(m_iGraphSize);
  }
  ...
  inline const int* operator[](int index) const {
    return m_pNeighborhoodGraph[index];
  }

  inline void SetR(int rows) {
 ...
 public:
  ...
  int m_iNeighborhoodSize, m_iNeighborhoodScale, m_iCEFScale, m_iRefineIter,
      m_iCEF, m_iMaxCheckForRefineGraph;
};
}
}
#endif
