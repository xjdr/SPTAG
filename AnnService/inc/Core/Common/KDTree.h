...
class KDTree {
 public:
  ...
  inline const KDTNode& operator[](int index) const {
...
#pragma omp parallel for
    for (int i = 0; i < m_iTreeNumber; i++) {
      ...
      DivideTree<T>(p_index, pindices, 0, (int)pindices.size() - 1,
                    m_pTreeStart[i], iTreeSize);
      std::cout << i + 1 << " KDTree built, " << iTreeSize - m_pTreeStart[i]
                << " " << pindices.size() << std::endl;
    }
  }
  ...
  bool SaveTrees(std::string sTreeFileName) const {
    ...
    fclose(fp);
    std::cout << "Save KDT (" << m_iTreeNumber << "," << treeNodeSize
              << ") Finish!" << std::endl;
    return true;
  }
  ...
  bool LoadTrees(std::string sTreeFileName) {
    ...
    fclose(fp);
    std::cout << "Load KDT (" << m_iTreeNumber << "," << treeNodeSize
              << ") Finish!" << std::endl;
    return true;
  }
  ...
  void InitSearchTrees(const VectorIndex* p_index,
    ...
    while (!p_space.m_SPTQueue.empty() &&
           p_space.m_iNumberOfCheckedLeaves < p_limits) {
 ...
 private:
  template <typename T>
  void KDTSearch(const VectorIndex* p_index,
                 const COMMON::QueryResultSet<T>& p_query,
                 COMMON::WorkSpace& p_space, const int node, const bool isInit,
                 const float distBound) const {
    if (node < 0) {
      int index = -node - 1;
      ...
      p_space.m_NGQueue.insert(COMMON::HeapCell(
          index, p_index->ComputeDistance((const void*)p_query.GetTarget(),
                                          (const void*)data)));
      return;
    }
  ...
  template <typename T>
  void DivideTree(VectorIndex* p_index, std::vector<int>& indices, int first,
                  int last, int index, int& iTreeSize) {
  ...
  template <typename T>
  void ChooseDivision(VectorIndex* p_index, KDTNode& node,
                      const std::vector<int>& indices, const int first,
                      const int last) {
    ...
    // choose the split dimension as one of the dimension inside TOP_DIM maximum
    // variance
  ...
  int SelectDivisionDimension(const std::vector<float>& varianceValues) const {
    ...
    for (int i = 0; i < varianceValues.size(); i++) {
      if (num < m_numTopDimensionKDTSplit ||
          varianceValues[i] > varianceValues[topind[num - 1]]) {
        if (num < m_numTopDimensionKDTSplit) {
          topind[num++] = i;
        } else {
          topind[num - 1] = i;
        ...
        while (j > 0 &&
               varianceValues[topind[j]] > varianceValues[topind[j - 1]]) {
  ...
  template <typename T>
  int Subdivide(VectorIndex* p_index, const KDTNode& node,
                std::vector<int>& indices, const int first,
