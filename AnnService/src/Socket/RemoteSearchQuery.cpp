...
RemoteSearchResult& RemoteSearchResult::operator=(
    RemoteSearchResult&& p_right) {
  m_status = p_right.m_status;
  m_allIndexResults = std::move(p_right.m_allIndexResults);
  ...
  for (const auto& indexRes : m_allIndexResults) {
    ...
    if (indexRes.m_results.WithMeta()) {
      for (int i = 0; i < indexRes.m_results.GetResultNum(); ++i) {
        sum += SimpleSerialization::EstimateBufferSize(
            indexRes.m_results.GetMetadata(i));
      }
    }
  }
...
std::uint8_t* RemoteSearchResult::Write(std::uint8_t* p_buffer) const {
  ...
  p_buffer = SimpleSerialization::SimpleWriteBuffer(
      static_cast<std::uint32_t>(m_allIndexResults.size()), p_buffer);
  for (const auto& indexRes : m_allIndexResults) {
    p_buffer =
        SimpleSerialization::SimpleWriteBuffer(indexRes.m_indexName, p_buffer);

    p_buffer = SimpleSerialization::SimpleWriteBuffer(
        static_cast<std::uint32_t>(indexRes.m_results.GetResultNum()),
        p_buffer);
    p_buffer = SimpleSerialization::SimpleWriteBuffer(
        indexRes.m_results.WithMeta(), p_buffer);
    ...
    if (indexRes.m_results.WithMeta()) {
      for (int i = 0; i < indexRes.m_results.GetResultNum(); ++i) {
        p_buffer = SimpleSerialization::SimpleWriteBuffer(
            indexRes.m_results.GetMetadata(i), p_buffer);
      }
    }
  }
...
const std::uint8_t* RemoteSearchResult::Read(const std::uint8_t* p_buffer) {
  ...
  for (auto& indexRes : m_allIndexResults) {
    p_buffer =
        SimpleSerialization::SimpleReadBuffer(p_buffer, indexRes.m_indexName);
