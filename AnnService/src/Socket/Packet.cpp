...
std::size_t PacketHeader::WriteBuffer(std::uint8_t* p_buffer) {
...
void PacketHeader::ReadBuffer(const std::uint8_t* p_buffer) {
...
PacketHeader& Packet::Header() { return m_header; }

std::uint8_t* Packet::HeaderBuffer() const { return m_buffer.get(); }

std::uint8_t* Packet::Body() const {
...
std::uint8_t* Packet::Buffer() const { return m_buffer.get(); }

std::uint32_t Packet::BufferLength() const {
  return PacketHeader::c_bufferSize + m_header.m_bodyLength;
}

std::uint32_t Packet::BufferCapacity() const { return m_bufferCapacity; }

void Packet::AllocateBuffer(std::uint32_t p_bodyCapacity) {
  m_bufferCapacity = PacketHeader::c_bufferSize + p_bodyCapacity;
  m_buffer.reset(new std::uint8_t[m_bufferCapacity],
                 std::default_delete<std::uint8_t[]>());
}

bool PacketTypeHelper::IsRequestPacket(PacketType p_type) {
  if (PacketType::Undefined == p_type || PacketType::ResponseMask == p_type) {
    return false;
  }

  return (static_cast<std::uint8_t>(p_type) &
          static_cast<std::uint8_t>(PacketType::ResponseMask)) == 0;
}

bool PacketTypeHelper::IsResponsePacket(PacketType p_type) {
  if (PacketType::Undefined == p_type || PacketType::ResponseMask == p_type) {
    return false;
  }
...
PacketType PacketTypeHelper::GetCrosspondingResponseType(PacketType p_type) {
  if (PacketType::Undefined == p_type || PacketType::ResponseMask == p_type) {
    return PacketType::Undefined;
  }

  auto ret = static_cast<std::uint8_t>(p_type) |
             static_cast<std::uint8_t>(PacketType::ResponseMask);
