...
ConnectionManager::ConnectionManager()
    : m_nextConnectionID(1), m_connectionCount(0) {}
...
ConnectionID ConnectionManager::AddConnection(
  ...
  while (c_invalidConnectionID == currID ||
         !m_connections[GetPosition(currID)].m_isEmpty.exchange(false)) {
  ...
  auto connection = std::make_shared<Connection>(
      currID, std::move(p_socket), p_handler,
...
void ConnectionManager::SetEventOnRemoving(
