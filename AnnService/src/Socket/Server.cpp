...
Server::Server(const std::string& p_address, const std::string& p_port,
  ...
  if (errCode) {
    fprintf(stderr, "Failed to resolve %s %s, error: %s", p_address.c_str(),
            p_port.c_str(), errCode.message().c_str());
  ...
  if (errCode) {
    fprintf(stderr, "Failed to bind %s %s, error: %s", p_address.c_str(),
            p_port.c_str(), errCode.message().c_str());
  ...
  if (errCode) {
    fprintf(stderr, "Failed to listen %s %s, error: %s", p_address.c_str(),
            p_port.c_str(), errCode.message().c_str());
  ...
  for (std::size_t i = 0; i < p_threadNum; ++i) {
    m_threadPool.emplace_back(
        std::move(std::thread([this]() { StartListen(); })));
  }
}
...
void Server::SetEventOnConnectionClose(
...
void Server::StartListen() { m_ioContext.run(); }

void Server::SendPacket(ConnectionID p_connection, Packet p_packet,
