...
ConnectionID Connection::GetConnectionID() const { return c_connectionID; }

ConnectionID Connection::GetRemoteConnectionID() const {
  return m_remoteConnectionID;
}

void Connection::AsyncSend(Packet p_packet,
  ...
  boost::asio::post(m_strand, [sharedThis, p_packet, p_callback]() {
    auto handler = [p_callback, p_packet, sharedThis](
        boost::system::error_code p_ec, std::size_t p_bytesTransferred) {
    ...
    boost::asio::async_write(
        sharedThis->m_socket,
        boost::asio::buffer(p_packet.Buffer(), p_packet.BufferLength()),
        std::move(handler));
  });
}
...
void Connection::AsyncReadHeader() {
  ...
  boost::asio::post(m_strand, [sharedThis]() {
    auto handler = boost::bind(&Connection::HandleReadHeader, sharedThis,
...
void Connection::AsyncReadBody() {
  ...
  auto sharedThis = shared_from_this();
  boost::asio::post(m_strand, [sharedThis]() {
    auto handler = boost::bind(&Connection::HandleReadBody, sharedThis,
                               boost::asio::placeholders::error,
                               boost::asio::placeholders::bytes_transferred);
...
void Connection::HandleReadHeader(boost::system::error_code p_ec,
...
void Connection::HandleReadBody(boost::system::error_code p_ec,
                                std::size_t p_bytesTransferred) {
  if (!p_ec) {
...
void Connection::SendHeartbeat(std::size_t p_intervalSeconds) {
  ...
  m_heartbeatTimer.expires_from_now(
      boost::posix_time::seconds(p_intervalSeconds));
  m_heartbeatTimer.async_wait(boost::bind(
      &Connection::SendHeartbeat, shared_from_this(), p_intervalSeconds));
}
...
void Connection::HandleNoHandlerResponse() {
  ...
  Packet msg;
  msg.Header().m_packetType =
      PacketTypeHelper::GetCrosspondingResponseType(packetType);
