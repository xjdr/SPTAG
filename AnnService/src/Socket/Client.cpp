...
Client::Client(const PacketHandlerMapPtr& p_handlerMap, std::size_t p_threadNum,
  ...
  for (std::size_t i = 0; i < p_threadNum; ++i) {
    m_threadPool.emplace_back(
        std::move(std::thread([this]() { m_ioContext.run(); })));
  }
}
...
ConnectionID Client::ConnectToServer(const std::string& p_address,
  ...
  if (socket.is_open()) {
    ...
    return m_connectionManager->AddConnection(
        std::move(socket), c_requestHandlerMap, m_heartbeatIntervalSeconds);
  }
...
void Client::AsyncConnectToServer(const std::string& p_address,
                                  const std::string& p_port,
                                  ConnectCallback p_callback) {
  boost::asio::post(m_ioContext, [this, p_address, p_port, p_callback]() {
...
void Client::SendPacket(ConnectionID p_connection, Packet p_packet,
...
void Client::SetEventOnConnectionClose(
    std::function<void(ConnectionID)> p_event) {
  m_connectionManager->SetEventOnRemoving(std::move(p_event));
}

void Client::KeepIoContext() {
  if (m_stopped) {
    return;
  }
  ...
  m_deadlineTimer.async_wait(
      [this](boost::system::error_code p_ec) { this->KeepIoContext(); });
}
