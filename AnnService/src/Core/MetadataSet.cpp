...
ErrorCode MetadataSet::RefineMetadata(std::vector<int>& indices,
  ...
  std::ofstream metaIndexOut(p_folderPath + "metadataIndex.bin",
                             std::ios::binary);
  if (!metaOut.is_open() || !metaIndexOut.is_open())
  ...
  SPTAG::MetadataSet::MetaCopy(p_folderPath + "metadata.bin_tmp",
                               p_folderPath + "metadata.bin");
  return ErrorCode::Success;
}

ErrorCode MetadataSet::MetaCopy(const std::string& p_src,
                                const std::string& p_dst) {
...
FileMetadataSet::FileMetadataSet(const std::string& p_metafile,
                                 const std::string& p_metaindexfile)
    : m_metaFile(p_metafile), m_metaindexFile(p_metaindexfile) {
  ...
  if (!m_fp->is_open() || !fpidx.is_open()) {
    std::cerr << "ERROR: Cannot open meta files " << p_metafile << " and "
              << p_metaindexfile << "!" << std::endl;
    return;
  }
...
ByteArray FileMetadataSet::GetMetadata(IndexType p_vectorID) const {
  ...
  if (p_vectorID < (IndexType)m_count) {
    ...
    return ByteArray((std::uint8_t*)m_newdata.data() + startoff,
                     static_cast<SizeType>(bytes), false);
  }
}

SizeType FileMetadataSet::Count() const {
  return static_cast<SizeType>(m_pOffsets.size() - 1);
}

bool FileMetadataSet::Available() const {
  return m_fp && m_fp->is_open() && m_pOffsets.size() > 1;
}

void FileMetadataSet::AddBatch(MetadataSet& data) {
  for (int i = 0; i < static_cast<int>(data.Count()); i++) {
    ByteArray newdata = data.GetMetadata(i);
    m_newdata.insert(m_newdata.end(), newdata.Data(),
                     newdata.Data() + newdata.Length());
    m_pOffsets.push_back(m_pOffsets[m_pOffsets.size() - 1] + newdata.Length());
  }
}

ErrorCode FileMetadataSet::SaveMetadata(const std::string& p_metaFile,
                                        const std::string& p_metaindexFile) {
  ...
  dst.write((char*)m_pOffsets.data(),
            sizeof(std::uint64_t) * m_pOffsets.size());
  return ret;
}

MemMetadataSet::MemMetadataSet(ByteArray p_metadata, ByteArray p_offsets,
                               SizeType p_count)
    : m_metadataHolder(std::move(p_metadata)),
      m_offsetHolder(std::move(p_offsets)),
      m_count(p_count) {
  const std::uint64_t* newdata =
      reinterpret_cast<const std::uint64_t*>(m_offsetHolder.Data());
  m_offsets.insert(m_offsets.end(), newdata, newdata + p_count + 1);
}

MemMetadataSet::~MemMetadataSet() {}

ByteArray MemMetadataSet::GetMetadata(IndexType p_vectorID) const {
  if (static_cast<SizeType>(p_vectorID) < m_count) {
    return ByteArray(m_metadataHolder.Data() + m_offsets[p_vectorID],
                     static_cast<SizeType>(m_offsets[p_vectorID + 1] -
                                           m_offsets[p_vectorID]),
                     m_metadataHolder.DataHolder());
  } else if (p_vectorID < m_offsets.size() - 1) {
    return ByteArray((std::uint8_t*)m_newdata.data() + m_offsets[p_vectorID] -
                         m_offsets[m_count],
                     static_cast<SizeType>(m_offsets[p_vectorID + 1] -
                                           m_offsets[p_vectorID]),
                     false);
...
SizeType MemMetadataSet::Count() const { return m_count; }

bool MemMetadataSet::Available() const {
  return m_metadataHolder.Length() > 0 && m_offsetHolder.Length() > 0;
}

void MemMetadataSet::AddBatch(MetadataSet& data) {
  for (int i = 0; i < static_cast<int>(data.Count()); i++) {
    ByteArray newdata = data.GetMetadata(i);
    m_newdata.insert(m_newdata.end(), newdata.Data(),
                     newdata.Data() + newdata.Length());
...
ErrorCode MemMetadataSet::SaveMetadata(const std::string& p_metaFile,
                                       const std::string& p_metaindexFile) {
  std::ofstream outputStream;
  outputStream.open(p_metaFile, std::ios::binary);
  if (!outputStream.is_open()) {
    std::cerr << "Error: Failed to create file " << p_metaFile << "."
              << std::endl;
    return ErrorCode::FailedCreateFile;
  }

  outputStream.write(reinterpret_cast<const char*>(m_metadataHolder.Data()),
                     m_metadataHolder.Length());
  outputStream.write((const char*)m_newdata.data(),
                     sizeof(std::uint8_t) * m_newdata.size());
  outputStream.close();

  outputStream.open(p_metaindexFile, std::ios::binary);
  if (!outputStream.is_open()) {
    std::cerr << "Error: Failed to create file " << p_metaindexFile << "."
              << std::endl;
    return ErrorCode::FailedCreateFile;
  }
  ...
  outputStream.write(reinterpret_cast<const char*>(m_offsets.data()),
                     sizeof(std::uint64_t) * m_offsets.size());
  outputStream.close();
