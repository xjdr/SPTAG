...
namespace BKT {
template <typename T>
ErrorCode Index<T>::LoadIndexFromMemory(
    const std::vector<void*>& p_indexBlobs) {
  if (!m_pSamples.Load((char*)p_indexBlobs[0]))
    return ErrorCode::FailedParseValue;
  if (!m_pTrees.LoadTrees((char*)p_indexBlobs[1]))
    return ErrorCode::FailedParseValue;
  if (!m_pGraph.LoadGraph((char*)p_indexBlobs[2]))
    return ErrorCode::FailedParseValue;
...
#define DefineBKTParameter(VarName, VarType, DefaultValue, RepresentStr)       \
  SetParameter(                                                                \
      RepresentStr,                                                            \
      p_reader.GetParameter("Index", RepresentStr, std::string(#DefaultValue)) \
          .c_str());

#include "inc/Core/BKT/ParameterDefinitionList.h"
#undef DefineBKTParameter

  if (!m_pSamples.Load(p_folderPath + m_sDataPointsFilename))
    return ErrorCode::Fail;
  if (!m_pTrees.LoadTrees(p_folderPath + m_sBKTFilename))
    return ErrorCode::Fail;
  if (!m_pGraph.LoadGraph(p_folderPath + m_sGraphFilename))
    return ErrorCode::Fail;

  m_workSpacePool.reset(
      new COMMON::WorkSpacePool(m_iMaxCheck, GetNumSamples()));
    ...
    while (!p_space.m_NGQueue.empty()) {                                     \
        ...
        if (p_query.AddPoint(gnode.node, gnode.distance)) {                  \
          ...
          if (checkNode < -1) {                                              \
            ...
            for (int i = -tnode.childStart; i < tnode.childEnd; i++) {       \
              if (!p_query.AddPoint(m_pTrees[i].centerid, gnode.distance))   \
                break;                                                       \
            }                                                                \
          }                                                                  \
        } else {                                                             \
      ...
      for (int i = 0; i <= checkPos; i++) {                                  \
        ...
        float distance2leaf = m_fComputeDistance(                            \
            p_query.GetTarget(), (m_pSamples)[nn_index], GetFeatureDim());   \
        p_space.m_iNumberOfCheckedLeaves++;                                  \
        p_space.m_NGQueue.insert(COMMON::HeapCell(nn_index, distance2leaf)); \
      }                                                                      \
      if (p_space.m_NGQueue.Top().distance >                                 \
...
template <typename T>
void Index<T>::SearchIndexWithDeleted(
...
template <typename T>
void Index<T>::SearchIndexWithoutDeleted(COMMON::QueryResultSet<T>& p_query,
                                         COMMON::WorkSpace& p_space) const {
  Search(;)
}

template <typename T>
ErrorCode Index<T>::SearchIndex(QueryResult& p_query) const {
  ...
  if (m_deletedID.size() > 0)
    ...
    SearchIndexWithoutDeleted(*((COMMON::QueryResultSet<T>*)&p_query),
                              *workSpace);
  ...
  if (p_query.WithMeta() && nullptr != m_pMetadata) {
    for (int i = 0; i < p_query.GetResultNum(); ++i) {
      ...
      p_query.SetMetadata(i, (result < 0) ? ByteArray::c_empty
                                          : m_pMetadata->GetMetadata(result));
    }
  }
  return ErrorCode::Success;
}
...
template <typename T>
ErrorCode Index<T>::BuildIndex(const void* p_data, int p_vectorNum,
                               int p_dimension) {
...
ErrorCode Index<T>::RefineIndex(const std::string& p_folderPath) {
  ...
  for (int i = 0; i < newR; i++) {
    if (m_deletedID.find(i) == m_deletedID.end()) {
      ...
      while (m_deletedID.find(newR - 1) != m_deletedID.end() && newR > i)
        newR--;
      if (newR == i) break;
  ...
  std::cout << "Refine... from " << GetNumSamples() << "->" << newR
            << std::endl;

  if (false == m_pSamples.Refine(indices, folderPath + m_sDataPointsFilename))
    return ErrorCode::FailedCreateFile;
  if (nullptr != m_pMetadata &&
      ErrorCode::Success != m_pMetadata->RefineMetadata(indices, folderPath))
    return ErrorCode::FailedCreateFile;
...
#pragma omp parallel for
  ...
  m_pGraph.RefineGraph<T>(this, indices, reverseIndices,
                          folderPath + m_sGraphFilename,
...
#pragma omp parallel for schedule(dynamic)
  for (int i = 0; i < p_vectorNum; i++) {
    COMMON::QueryResultSet<T> query(ptr_v + i * GetFeatureDim(),
                                    m_pGraph.m_iCEF);
    SearchIndex(query);
...
template <typename T>
ErrorCode Index<T>::AddIndex(const void* p_vectors, int p_vectorNum,
                             int p_dimension) {
  int begin, end;
  {
    std::lock_guard<std::mutex> lock(m_dataLock);
...
#undef DefineBKTParameter
  ...
  if (m_deletedID.size() > 0) {
    ...
    if (!m_pSamples.Save(p_folderPath + m_sDataPointsFilename))
      return ErrorCode::Fail;
    if (!m_pTrees.SaveTrees(p_folderPath + m_sBKTFilename))
      return ErrorCode::Fail;
    if (!m_pGraph.SaveGraph(p_folderPath + m_sGraphFilename))
      return ErrorCode::Fail;
  }
  return ErrorCode::Success;
}
...
#define DefineBKTParameter(VarName, VarType, DefaultValue, RepresentStr)  \
  else if (SPTAG::Helper::StrUtils::StrEqualIgnoreCase(p_param,           \
                                                       RepresentStr)) {   \
...
template <typename T>
std::string Index<T>::GetParameter(const char* p_param) const {
