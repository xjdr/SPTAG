...
WorkSpacePool::WorkSpacePool(int p_maxCheck, int p_vectorCount)
    : m_maxCheck(p_maxCheck), m_vectorCount(p_vectorCount) {}

WorkSpacePool::~WorkSpacePool() {
  for (auto& workSpace : m_workSpacePool) workSpace.reset();
  m_workSpacePool.clear();
}

std::shared_ptr<WorkSpace> WorkSpacePool::Rent() {
...
void WorkSpacePool::Return(const std::shared_ptr<WorkSpace>& p_workSpace) {
  {
    std::lock_guard<std::mutex> lock(m_workSpacePoolMutex);
...
void WorkSpacePool::Init(int size) {
