...
#undef DefineKDTParameter
  ...
  if (!m_pTrees.LoadTrees(p_folderPath + m_sKDTFilename))
    return ErrorCode::Fail;
...
#define Search(CheckDeleted1)                                                  \
  ...
  while (!p_space.m_NGQueue.empty()) {                                         \
    ...
    if (p_space.m_iNumOfContinuousNoBetterPropagation >                        \
      ...
      if (p_space.m_iNumberOfTreeCheckedLeaves <=                              \
          p_space.m_iNumberOfCheckedLeaves / 10) {                             \
...
#undef DefineKDTParameter
  ...
  if (m_deletedID.size() > 0) {
    ...
    if (!m_pSamples.Save(p_folderPath + m_sDataPointsFilename))
      return ErrorCode::Fail;
    if (!m_pTrees.SaveTrees(p_folderPath + m_sKDTFilename))
      return ErrorCode::Fail;
