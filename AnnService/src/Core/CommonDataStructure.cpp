...
ByteArray::ByteArray() : m_data(nullptr), m_length(0) {}
...
ByteArray::ByteArray(std::uint8_t* p_array, std::size_t p_length,
                     bool p_transferOnwership)
    : m_data(p_array), m_length(p_length) {
...
ByteArray& ByteArray::operator=(const ByteArray& p_right) {
  m_data = p_right.m_data;
  m_length = p_right.m_length;
  m_dataHolder = p_right.m_dataHolder;
...
ByteArray& ByteArray::operator=(ByteArray&& p_right) {
  m_data = p_right.m_data;
  m_length = p_right.m_length;
  m_dataHolder = std::move(p_right.m_dataHolder);
...
ByteArray ByteArray::Alloc(std::size_t p_length) {
  ByteArray byteArray;
  if (0 == p_length) {
    return byteArray;
  }
...
std::uint8_t* ByteArray::Data() const { return m_data; }

std::size_t ByteArray::Length() const { return m_length; }

void ByteArray::SetData(std::uint8_t* p_array, std::size_t p_length) {
  m_data = p_array;
  m_length = p_length;
}

std::shared_ptr<std::uint8_t> ByteArray::DataHolder() const {
  return m_dataHolder;
}
