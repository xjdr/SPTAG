...
void AggregatorService::Run() {
  auto threadNum =
      max((SPTAG::SizeType)1, GetContext()->GetSettings()->m_threadNum);
...
void AggregatorService::StartClient() {
  ...
  handlerMap->emplace(
      Socket::PacketType::SearchResponse,
      [this](Socket::ConnectionID p_srcID, Socket::Packet p_packet) {
        boost::asio::post(*m_threadPool,
                          std::bind(&AggregatorService::SearchResponseHanlder,
                                    this, p_srcID, std::move(p_packet)));
      });

  m_socketClient.reset(new Socket::Client(
      handlerMap, context->GetSettings()->m_socketThreadNum, 30));
...
void AggregatorService::StartListen() {
  ...
  Socket::PacketHandlerMapPtr handlerMap(new Socket::PacketHandlerMap);
  handlerMap->emplace(
      Socket::PacketType::SearchRequest,
      [this](Socket::ConnectionID p_srcID, Socket::Packet p_packet) {
        boost::asio::post(*m_threadPool,
  ...
  m_socketServer.reset(
      new Socket::Server(context->GetSettings()->m_listenAddr,
                         context->GetSettings()->m_listenPort, handlerMap,
                         context->GetSettings()->m_socketThreadNum));

  fprintf(stderr, "Start to listen %s:%s ...\n",
...
void AggregatorService::WaitForShutdown() {
  ...
  m_shutdownSignals.async_wait(
      [this](boost::system::error_code p_ec, int p_signal) {
...
void AggregatorService::ConnectToPendingServers() {
  ...
  for (auto& pendingServer : pendingList) {
    ...
    auto runner = [server, this]() {
      ErrorCode errCode;
      auto cid = m_socketClient->ConnectToServer(server->m_address,
                                                 server->m_port, errCode);
  ...
  m_pendingConnectServersTimer.async_wait(
      [this](const boost::system::error_code& p_ec) {
...
void AggregatorService::AddToPendingServers(
    std::shared_ptr<RemoteMachine> p_remoteServer) {
  std::lock_guard<std::mutex> guard(m_pendingConnectServersMutex);
  m_pendingConnectServers.emplace_back(std::move(p_remoteServer));
}

void AggregatorService::SearchRequestHanlder(
    Socket::ConnectionID p_localConnectionID, Socket::Packet p_packet) {
  auto context = GetContext();
  ...
  for (std::uint32_t i = 0; i < remoteServers.size(); ++i) {
    AggregatorCallback callback = [this, executionContext,
                                   i](Socket::RemoteSearchResult p_result) {
      executionContext->GetResult(i).reset(
          new Socket::RemoteSearchResult(std::move(p_result)));
    ...
    auto connectCallback = [callback](bool p_connectSucc) {
      if (!p_connectSucc) {
        Socket::RemoteSearchResult result;
        result.m_status =
            Socket::RemoteSearchResult::ResultStatus::FailedNetwork;
    ...
    packet.Header().m_resourceID = m_aggregatorCallbackManager.Add(
        std::make_shared<AggregatorCallback>(std::move(callback)),
    ...
    m_socketClient->SendPacket(remoteServers[i], std::move(packet),
                               connectCallback);
  }
}

void AggregatorService::SearchResponseHanlder(
    Socket::ConnectionID p_localConnectionID, Socket::Packet p_packet) {
  auto callback =
      m_aggregatorCallbackManager.GetAndRemove(p_packet.Header().m_resourceID);
  if (nullptr == callback) {
    return;
  }

  if (p_packet.Header().m_processStatus != Socket::PacketProcessStatus::Ok ||
      0 == p_packet.Header().m_bodyLength) {
    Socket::RemoteSearchResult result;
...
void AggregatorService::AggregateResults(
    std::shared_ptr<AggregatorExecutionContext> p_exectionContext) {
  ...
  packet.Header().m_resourceID =
      p_exectionContext->GetRequestHeader().m_resourceID;
  ...
  std::uint32_t cap =
      static_cast<std::uint32_t>(remoteResult.EstimateBufferSize());
  packet.AllocateBuffer(cap);
  packet.Header().m_bodyLength = static_cast<std::uint32_t>(
      remoteResult.Write(packet.Body()) - packet.Body());
  packet.Header().WriteBuffer(packet.HeaderBuffer());

  m_socketServer->SendPacket(
      p_exectionContext->GetRequestHeader().m_connectionID, std::move(packet),
      nullptr);
}
