...
AggregatorContext::AggregatorContext(const std::string& p_filePath)
  ...
  m_settings->m_listenPort =
      iniReader.GetParameter("Service", "ListenPort", std::string("8100"));
  ...
  std::uint32_t serverNum = iniReader.GetParameter(
      "Servers", "Number", static_cast<std::uint32_t>(0));
  ...
  for (std::uint32_t i = 0; i < serverNum; ++i) {
    ...
    remoteMachine->m_address =
        iniReader.GetParameter(sectionName, "Address", emptyStr);
    remoteMachine->m_port =
        iniReader.GetParameter(sectionName, "Port", emptyStr);
...
const std::shared_ptr<AggregatorSettings>& AggregatorContext::GetSettings()
    const {
  return m_settings;
}
