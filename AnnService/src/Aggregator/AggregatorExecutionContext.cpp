...
AggregatorExecutionContext::AggregatorExecutionContext(
    std::size_t p_totalServerNumber, Socket::PacketHeader p_requestHeader)
...
std::size_t AggregatorExecutionContext::GetServerNumber() const {
  return m_results.size();
}

AggregatorResult& AggregatorExecutionContext::GetResult(std::size_t p_num) {
  return m_results[p_num];
}

const Socket::PacketHeader& AggregatorExecutionContext::GetRequestHeader()
    const {
  return m_requestHeader;
}

bool AggregatorExecutionContext::IsCompletedAfterFinsh(
    std::uint32_t p_finishedCount) {
