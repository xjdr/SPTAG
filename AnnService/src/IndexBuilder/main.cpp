...
int main(int argc, char* argv[]) {
  std::shared_ptr<IndexBuilder::BuilderOptions> options(
      new IndexBuilder::BuilderOptions);
  if (!options->Parse(argc - 1, argv + 1)) {
    exit(1);
  }
  IndexBuilder::ThreadPool::Init(options->m_threadNum);
  auto indexBuilder = VectorIndex::CreateInstance(options->m_indexAlgoType,
  ...
  if (!iniReader.DoesParameterExist("Index", "NumberOfThreads")) {
    iniReader.SetParameter("Index", "NumberOfThreads",
                           std::to_string(options->m_threadNum));
  }
  ...
  if (options->m_inputFiles.find("BIN:") == 0) {
    std::vector<std::string> files = SPTAG::Helper::StrUtils::SplitString(
        options->m_inputFiles.substr(4), ",");
    ...
    std::uint64_t totalRecordVectorBytes =
        ((std::uint64_t)GetValueTypeSize(options->m_inputValueType)) * row *
        col;
    ...
    std::shared_ptr<VectorSet> p_vectorSet(
        new BasicVectorSet(vectorSet, options->m_inputValueType, col, row));
    ...
    code = indexBuilder->BuildIndex(vectorReader->GetVectorSet(),
