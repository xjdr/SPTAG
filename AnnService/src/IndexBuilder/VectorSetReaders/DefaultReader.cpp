...
class BinaryLineReader {
 public:
  BinaryLineReader(std::istream& p_inStream) : m_inStream(p_inStream) {
    m_buffer.reset(new char[c_bufferSize]);
  }

  bool Eof() { return m_inStream.eof() && (m_curOffset == m_curTotal); }

  std::size_t GetLine(std::unique_ptr<char[]>& p_buffer,
                      std::size_t& p_bufferSize, std::size_t& p_length) {
 ...
 private:
  ...
  bool IsDelimiter(char p_ch) { return p_ch == '\r' || p_ch == '\n'; }
...
DefaultReader::DefaultReader(std::shared_ptr<BuilderOptions> p_options)
    : VectorSetReader(std::move(p_options)), m_subTaskBlocksize(0) {
...
ErrorCode DefaultReader::LoadFile(const std::string& p_filePaths) {
  ...
  for (const auto& fileInfo : files) {
    ...
    if (0 == blockSize) {
      ...
      fileTaskCount = static_cast<std::uint32_t>(
          (fileInfo.second + blockSize - 1) / blockSize);
    }

    for (std::uint32_t i = 0; i < fileTaskCount; ++i) {
      subWorks.emplace_back(std::bind(&DefaultReader::LoadFileInternal, this,
                                      fileInfo.first, m_subTaskCount++, i,
                                      blockSize));
    }
  }
...
std::shared_ptr<VectorSet> DefaultReader::GetVectorSet() const {
  ...
  return std::shared_ptr<VectorSet>(
      new BasicVectorSet(vectorSet, m_options->m_inputValueType,
                         m_options->m_dimension, m_totalRecordCount));
}

std::shared_ptr<MetadataSet> DefaultReader::GetMetadataSet() const {
  return std::shared_ptr<MetadataSet>(
      new FileMetadataSet(m_metadataConentOutput, m_metadataIndexOutput));
}

void DefaultReader::LoadFileInternal(const std::string& p_filePath,
  ...
  {
    std::stringstream msg;
    msg << "Begin Subtask: " << p_subTaskID
        << ", start offset position:" << startpos << std::endl;
    std::cout << msg.str();
  }
  ...
  metaStreamContent.open(m_metadataConentOutput + subFileSuffix,
                         std::ofstream::binary);
  metaStreamIndex.open(m_metadataIndexOutput + subFileSuffix,
                       std::ofstream::binary);
  ...
  std::size_t vectorByteSize =
      GetValueTypeSize(m_options->m_inputValueType) * m_options->m_dimension;
  ...
  while (!lineReader.Eof() && totalRead <= p_fileBlockSize) {
    ...
    if (0 == tabIndex && currentLine[tabIndex] != '\t') {
      std::stringstream msg;
      msg << "Subtask: " << p_subTaskID
          << " cannot parsing line:" << currentLine.get() << std::endl;
      std::cout << msg.str();
      exit(1);
    }
...
#define DefineVectorValueType(Name, Type)                                  \
  case VectorValueType::Name:                                              \
    parseSuccess = TranslateVector(currentLine.get() + tabIndex + 1,       \
                                   reinterpret_cast<Type*>(vector.get())); \
    break;
...
#undef DefineVectorValueType
    ...
    if (!parseSuccess) {
      std::stringstream msg;
      msg << "Subtask: " << p_subTaskID
          << " cannot parsing vector:" << (currentLine.get() + tabIndex + 1)
          << std::endl;
      std::cout << msg.str();
      exit(1);
    }
    ...
    ++recordCount;
    outputStream.write(reinterpret_cast<const char*>(vector.get()),
                       vectorByteSize);
    metaStreamContent.write(currentLine.get(), tabIndex);
    metaStreamIndex.write(reinterpret_cast<const char*>(&metaOffset),
                          sizeof(metaOffset));
...
void DefaultReader::MergeData() {
  const std::size_t bufferSize = 1 << 30;
  const std::size_t bufferSizeTrim64 =
      (bufferSize / sizeof(std::uint64_t)) * sizeof(std::uint64_t);
  std::ifstream inputStream;
  std::ofstream outputStream;
  ...
  outputStream.write(reinterpret_cast<char*>(&(m_options->m_dimension)),
                     sizeof(m_options->m_dimension));
  ...
  for (std::uint32_t i = 0; i < m_subTaskCount; ++i) {
    ...
    for (std::uint32_t remains = m_subTaskRecordCount[i]; remains > 0;) {
      std::size_t readBytesCount =
          min(remains * sizeof(std::uint64_t), bufferSizeTrim64);
      inputStream.read(buf, readBytesCount);
      std::uint64_t* offset = reinterpret_cast<std::uint64_t*>(buf);
      for (std::uint32_t i = 0; i < readBytesCount / sizeof(std::uint64_t);
           ++i) {
      ...
      remains -=
          static_cast<std::uint32_t>(readBytesCount / sizeof(std::uint64_t));
    }
  ...
  outputStream.write(reinterpret_cast<char*>(&totalOffset),
                     sizeof(totalOffset));
  outputStream.close();
}

std::vector<DefaultReader::FileInfoPair> DefaultReader::GetFileSizes(
    const std::string& p_filePaths) {
