...
ErrorCode IniReader::LoadIniFile(const std::string& p_iniFilePath) {
  ...
  while (!input.eof()) {
    ...
    auto nonSpaceSeg =
        StrUtils::FindTrimmedSegment(line.get(), line.get() + len, isSpace);
    ...
    if (';' == *nonSpaceSeg.first) {
      ...
      auto sectionSeg = StrUtils::FindTrimmedSegment(
          nonSpaceSeg.first + 1, nonSpaceSeg.second - 1, isSpace);
      ...
      auto paramSeg = StrUtils::FindTrimmedSegment(nonSpaceSeg.first,
                                                   equalSignLoc, isSpace);
      ...
      if (currParamMap->count(paramName) == 0) {
        currParamMap->emplace(
            std::move(paramName),
            std::string(equalSignLoc + 1, nonSpaceSeg.second));
      } else {
...
bool IniReader::DoesParameterExist(const std::string& p_section,
...
bool IniReader::GetRawValue(const std::string& p_section,
...
const IniReader::ParameterValueMap& IniReader::GetParameters(
    const std::string& p_section) const {
  std::string name(p_section);
  StrUtils::ToLowerInPlace(name);
  auto sectionIter = m_parameters.find(name);
...
void IniReader::SetParameter(const std::string& p_section,
                             const std::string& p_param,
  ...
  if (sectionIter == m_parameters.cend() || sectionIter->second == nullptr) {
    m_parameters[name] =
        std::shared_ptr<ParameterValueMap>(new ParameterValueMap);
  }
