...
ClientWrapper::ClientWrapper(const ClientOptions& p_options)
  ...
  m_client.reset(new SPTAG::Socket::Client(GetHandlerMap(),
                                           p_options.m_socketThreadNum, 30));
  m_client->SetEventOnConnectionClose(std::bind(
      &ClientWrapper::HandleDeadConnection, this, std::placeholders::_1));
  ...
  for (std::uint32_t i = 0; i < m_options.m_threadNum; ++i) {
    ...
    conn.first = m_client->ConnectToServer(p_options.m_serverAddr,
                                           p_options.m_serverPort, errCode);
...
void ClientWrapper::SendQueryAsync(const Socket::RemoteQuery& p_query,
                                   Callback p_callback,
  ...
  packet.Header().m_resourceID = m_callbackManager.Add(
      std::make_shared<Callback>(std::move(p_callback)),
      p_options.m_searchTimeout, std::move(timeoutCallback));

  packet.Header().m_bodyLength =
      static_cast<std::uint32_t>(p_query.EstimateBufferSize());
...
PacketHandlerMapPtr ClientWrapper::GetHandlerMap() {
  ...
  handlerMap->emplace(PacketType::SearchResponse,
                      std::bind(&ClientWrapper::SearchResponseHanlder, this,
...
const ClientWrapper::ConnectionPair& ClientWrapper::GetConnection() {
  ...
  std::uint32_t pos =
      m_spinCountOfConnection.fetch_add(1) % m_connections.size();
  while (c_invalidConnectionID == m_connections[pos].first &&
         triedCount < m_connections.size()) {
...
void ClientWrapper::SearchResponseHanlder(
    Socket::ConnectionID p_localConnectionID, Socket::Packet p_packet) {
  std::shared_ptr<Callback> callback =
      m_callbackManager.GetAndRemove(p_packet.Header().m_resourceID);
  if (nullptr == callback) {
    return;
  }

  if (p_packet.Header().m_processStatus != PacketProcessStatus::Ok ||
  ...
  for (auto& conn : m_connections) {
    if (conn.first == p_cid) {
      ...
      while (c_invalidConnectionID == conn.first) {
        conn.first = m_client->ConnectToServer(m_options.m_serverAddr,
                                               m_options.m_serverPort, errCode);
        if (SPTAG::ErrorCode::Socket_FailedResolveEndPoint == errCode) {
          break;
        }
      }
...
bool ClientWrapper::IsAvailable() const { return !m_connections.empty(); }
