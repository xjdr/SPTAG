...
class SerivceCmdOptions : public Helper::ArgumentsParser {
 public:
  SerivceCmdOptions()
      : m_serveMode("interactive"), m_configFile("AnnService.ini") {
    AddOptionalOption(m_serveMode, "-m", "--mode",
                      "Service mode, interactive or socket.");
    AddOptionalOption(m_configFile, "-c", "--config",
...
bool SearchService::Initialize(int p_argNum, char* p_args[]) {
  ...
  if (Helper::StrUtils::StrEqualIgnoreCase(cmdOptions.m_serveMode.c_str(),
                                           "interactive")) {
    m_serveMode = ServeMode::Interactive;
  } else if (Helper::StrUtils::StrEqualIgnoreCase(
                 cmdOptions.m_serveMode.c_str(), "socket")) {
...
void SearchService::RunSocketMode() {
  auto threadNum =
      max((unsigned int)1, m_serviceContext->GetServiceSettings()->m_threadNum);
  ...
  handlerMap->emplace(
      ...
      [this](Socket::ConnectionID p_srcID, Socket::Packet p_packet) {
        boost::asio::post(*m_threadPool,
                          std::bind(&SearchService::SearchHanlder, this,
                                    p_srcID, std::move(p_packet)));
      });

  m_socketServer.reset(new Socket::Server(
      m_serviceContext->GetServiceSettings()->m_listenAddr,
...
void SearchService::SearchHanlder(Socket::ConnectionID p_localConnectionID,
  ...
  auto callback = std::bind(&SearchService::SearchHanlderCallback, this,
                            std::placeholders::_1, std::move(p_packet));

  SearchExecutor executor(std::move(remoteQuery.m_queryString),
                          m_serviceContext, callback);
  executor.Execute();
}

void SearchService::SearchHanlderCallback(
  ...
  if (nullptr == p_exeContext) {
    ...
    ret.Header().m_bodyLength =
        static_cast<std::uint32_t>(bodyEnd - ret.Body());
    ret.Header().WriteBuffer(ret.HeaderBuffer());
  }

  m_socketServer->SendPacket(p_srcPacket.Header().m_connectionID,
                             std::move(ret), nullptr);
}
