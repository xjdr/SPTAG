...
ErrorCode QueryParser::Parse(const std::string& p_query,
                             const char* p_vectorSeparator) {
  ...
  for (iter = reinterpret_cast<char*>(m_dataHolder.Data()); *iter != '\0';
       ++iter) {
    if (std::isspace(*iter)) {
      *iter = '\0';
...
const char* QueryParser::GetVectorBase64() const { return m_vectorBase64; }
