...
ErrorCode ConvertVectorFromString(const std::vector<const char*>& p_source,
                                  ByteArray& p_dest, SizeType& p_dimension) {
  p_dimension = 0;
...
SearchExecutionContext::~SearchExecutionContext() { m_results.clear(); }

ErrorCode SearchExecutionContext::ParseQuery(const std::string& p_query) {
  return m_queryParser.Parse(p_query,
                             c_serviceSettings->m_vectorSeparator.c_str());
}
...
ErrorCode SearchExecutionContext::ExtractOption() {
  for (const auto& optionPair : m_queryParser.GetOptions()) {
    ...
    } else if (Helper::StrUtils::StrEqualIgnoreCase(optionPair.first,
                                                    "datatype")) {
      Helper::Convert::ConvertStringTo<VectorValueType>(optionPair.second,
                                                        m_inputValueType);
    } else if (Helper::StrUtils::StrEqualIgnoreCase(optionPair.first,
                                                    "extractmetadata")) {
      Helper::Convert::ConvertStringTo<bool>(optionPair.second,
                                             m_extractMetadata);
    } else if (Helper::StrUtils::StrEqualIgnoreCase(optionPair.first,
                                                    "resultnum")) {
      Helper::Convert::ConvertStringTo<SizeType>(optionPair.second,
                                                 m_resultNum);
    }
  }
...
#undef DefineVectorValueType
    ...
    std::size_t outLen = 0;
    if (!Helper::Base64::Decode(m_queryParser.GetVectorBase64(), estLen,
                                temp.Data(), outLen)) {
      return ErrorCode::Fail;
    }
...
const std::vector<std::string>& SearchExecutionContext::GetSelectedIndexNames()
    const {
  return m_indexNames;
}

void SearchExecutionContext::AddResults(std::string p_indexName,
...
const ByteArray& SearchExecutionContext::GetVector() const { return m_vector; }

const SizeType SearchExecutionContext::GetVectorDimension() const {
  return m_vectorDimension;
}

const std::vector<QueryParser::OptionPair>& SearchExecutionContext::GetOptions()
    const {
