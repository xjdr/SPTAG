...
template <typename T>
float CalcRecall(std::vector<QueryResult>& results,
                 const std::vector<std::set<int>>& truth, int NumQuerys, int K,
                 std::ofstream& log) {
  ...
  log << meanrecall << " " << stdrecall << " " << minrecall << " " << maxrecall
      << std::endl;
  return meanrecall;
}

void LoadTruth(std::ifstream& fp, std::vector<std::set<int>>& truth,
               int NumQuerys, int K) {
  int get;
  std::string line;
  for (int i = 0; i < NumQuerys; ++i) {
...
int Process(Helper::IniReader& reader, VectorIndex& index) {
  std::string queryFile =
      reader.GetParameter("Index", "QueryFile", std::string("querys.bin"));
  std::string truthFile =
      reader.GetParameter("Index", "TruthFile", std::string("truth.txt"));
  std::string outputFile =
      reader.GetParameter("Index", "ResultFile", std::string(""));
  ...
  std::vector<std::string> maxCheck = Helper::StrUtils::SplitString(
      reader.GetParameter("Index", "MaxCheck", std::string("2048")), "#");

  std::ifstream inStream(queryFile);
  std::ifstream ftruth(truthFile);
  std::ofstream fp;
  if (!inStream.is_open()) {
    std::cout << "ERROR: Cannot Load Query file " << queryFile << "!"
              << std::endl;
    return -1;
  }
  if (outputFile != "") {
    fp.open(outputFile);
    if (!fp.is_open()) {
      std::cout << "ERROR: Cannot open " << outputFile << " for write!"
                << std::endl;
    }
  }
  ...
  std::vector<std::vector<T>> Query(numQuerys,
                                    std::vector<T>(index.GetFeatureDim(), 0));
  ...
  while (!inStream.eof()) {
    QStrings.clear();
    COMMON::Utils::PrepareQuerys(inStream, QStrings, Query, numQuerys, dims,
                                 index.GetDistCalcMethod(), base);
...
#pragma omp parallel for
      ...
      for (int i = 0; i < numQuerys; i++)
        timeStd +=
            ((float)latencies[i] - timeMean) * ((float)latencies[i] - timeMean);
      timeStd = std::sqrt(timeStd / numQuerys);
      log << timeMean << " " << timeStd << " " << timeMin << " " << timeMax
          << " ";

      std::sort(latencies, latencies + numQuerys,
...
#endif
      std::cout << mc << "\t" << std::fixed << std::setprecision(6)
                << (timeMean / CLOCKS_PER_SEC) << "\t" << std::setprecision(4)
                << l99 << "\t" << l95 << "\t" << recall << "\t\t" << peakWSS
                << "GB" << std::endl;
    }
      ...
      for (int i = 0; i < numQuerys; i++) {
        ...
        for (int j = 0; j < K; j++) {
          if (results[i].GetResult(j)->VID < 0) {
            fp << results[i].GetResult(j)->Dist << "@"
               << results[i].GetResult(j)->VID << std::endl;
          } else {
...
int main(int argc, char** argv) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    std::cout << "Set [" << sectionName << "]" << paramName << " = " << paramVal
              << std::endl;
  }
