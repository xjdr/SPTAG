...
T random(int high = RAND_MAX, int low = 0)  // Generates a random value.
{
  return (T)(low +
             float(high - low) *
                 (std::rand() / static_cast<float>(RAND_MAX + 1.0)));
}
...
void test(int high) {
  ...
  BOOST_CHECK_CLOSE_FRACTION(
      ...
      SPTAG::COMMON::DistanceUtils::ComputeL2Distance(X, Y, dimension), 1e-5);
  BOOST_CHECK_CLOSE_FRACTION(
      high * high - ComputeCosineDistance(X, Y, dimension),
      SPTAG::COMMON::DistanceUtils::ComputeCosineDistance(X, Y, dimension),
      1e-5);
