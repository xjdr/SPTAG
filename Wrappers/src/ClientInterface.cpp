...
AnnClient::AnnClient(const char* p_serverAddr, const char* p_serverPort)
  ...
  auto connectCallback = [this](Socket::ConnectionID p_cid, ErrorCode p_ec) {
    ...
    while (Socket::c_invalidConnectionID == m_connectionID) {
      ...
      m_connectionID =
          m_socketClient->ConnectToServer(m_server, m_port, errCode);
    }
  };
...
std::shared_ptr<RemoteSearchResult> AnnClient::Search(ByteArray p_data,
                                                      int p_resultNum,
  ...
  if (Socket::c_invalidConnectionID != m_connectionID) {
    ...
    SPTAG::Helper::Convert::ConvertStringTo<SPTAG::VectorValueType>(p_valueType,
                                                                    valueType);
    query.m_queryString =
        CreateSearchQuery(p_data, p_resultNum, p_withMetaData, valueType);

    packet.Header().m_bodyLength =
        static_cast<std::uint32_t>(query.EstimateBufferSize());
    packet.AllocateBuffer(packet.Header().m_bodyLength);
    query.Write(packet.Body());
    packet.Header().WriteBuffer(packet.HeaderBuffer());

    m_socketClient->SendPacket(m_connectionID, std::move(packet),
                               connectCallback);
...
SPTAG::Socket::PacketHandlerMapPtr AnnClient::GetHandlerMap() {
  ...
  handlerMap->emplace(Socket::PacketType::SearchResponse,
                      std::bind(&AnnClient::SearchResponseHanlder, this,
                                std::placeholders::_1, std::placeholders::_2));

  return handlerMap;
}

void AnnClient::SearchResponseHanlder(
    SPTAG::Socket::ConnectionID p_localConnectionID,
...
std::string AnnClient::CreateSearchQuery(const ByteArray& p_data,
                                         int p_resultNum,
