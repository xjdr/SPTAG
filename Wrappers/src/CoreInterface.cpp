...
AnnIndex::AnnIndex(const char* p_algoType, const char* p_valueType,
                   int p_dimension)
    : m_algoType(SPTAG::IndexAlgoType::Undefined),
      m_inputValueType(SPTAG::VectorValueType::Undefined),
      m_dimension(p_dimension) {
  SPTAG::Helper::Convert::ConvertStringTo<SPTAG::IndexAlgoType>(p_algoType,
                                                                m_algoType);
  SPTAG::Helper::Convert::ConvertStringTo<SPTAG::VectorValueType>(
      p_valueType, m_inputValueType);
  m_inputVectorSize = SPTAG::GetValueTypeSize(m_inputValueType) * m_dimension;
}
...
bool AnnIndex::Build(ByteArray p_data, int p_num) {
  if (nullptr == m_index) {
    m_index = SPTAG::VectorIndex::CreateInstance(m_algoType, m_inputValueType);
  }
  if (nullptr == m_index || p_num == 0 || m_dimension == 0 ||
      p_data.Length() != p_num * m_inputVectorSize) {
    return false;
  }
  return (SPTAG::ErrorCode::Success ==
          m_index->BuildIndex(p_data.Data(), p_num, m_dimension));
}

bool AnnIndex::BuildWithMetaData(ByteArray p_data, ByteArray p_meta,
                                 int p_num) {
  if (nullptr == m_index) {
    m_index = SPTAG::VectorIndex::CreateInstance(m_algoType, m_inputValueType);
  }
  ...
  std::shared_ptr<SPTAG::VectorSet> vectors(new SPTAG::BasicVectorSet(
      p_data, m_inputValueType, static_cast<int>(m_dimension),
  ...
  std::shared_ptr<SPTAG::MetadataSet> meta(new SPTAG::MemMetadataSet(
      p_meta, ByteArray((std::uint8_t*)offsets,
                        (p_num + 1) * sizeof(std::uint64_t), true),
      p_num));
  return (SPTAG::ErrorCode::Success == m_index->BuildIndex(vectors, meta));
}

void AnnIndex::SetBuildParam(const char* p_name, const char* p_value) {
  if (nullptr == m_index) {
...
void AnnIndex::SetSearchParam(const char* p_name, const char* p_value) {
  if (nullptr != m_index) m_index->SetParameter(p_name, p_value);
}

std::shared_ptr<QueryResult> AnnIndex::Search(ByteArray p_data,
                                              int p_resultNum) {
  std::shared_ptr<QueryResult> results =
      std::make_shared<QueryResult>(p_data.Data(), p_resultNum, false);
...
std::shared_ptr<QueryResult> AnnIndex::SearchWithMetaData(ByteArray p_data,
                                                          int p_resultNum) {
  std::shared_ptr<QueryResult> results =
      std::make_shared<QueryResult>(p_data.Data(), p_resultNum, true);
...
bool AnnIndex::ReadyToServe() const { return m_index != nullptr; }
...
bool AnnIndex::Add(ByteArray p_data, int p_num) {
  if (nullptr == m_index) {
    m_index = SPTAG::VectorIndex::CreateInstance(m_algoType, m_inputValueType);
  }
  if (nullptr == m_index || p_num == 0 || m_dimension == 0 ||
  ...
  return (SPTAG::ErrorCode::Success ==
          m_index->AddIndex(p_data.Data(), p_num, m_dimension));
}

bool AnnIndex::AddWithMetaData(ByteArray p_data, ByteArray p_meta, int p_num) {
  if (nullptr == m_index) {
    m_index = SPTAG::VectorIndex::CreateInstance(m_algoType, m_inputValueType);
  }
  if (nullptr == m_index || p_num == 0 || m_dimension == 0 ||
...
bool AnnIndex::Delete(ByteArray p_data, int p_num) {
  if (nullptr != m_index && p_num > 0) {
    return (SPTAG::ErrorCode::Success ==
            m_index->DeleteIndex(p_data.Data(), p_num));
  }
  return false;
}
